...
PetscErrorCode MMA::SetOuterMovelimitVec(Vec XMIN, Vec XMAX, PetscScalar movlim, Vec x, Vec xmin, Vec xmax){
	PetscErrorCode ierr=0;

	PetscScalar *xv,*xmiv,*xmav, *xMIv, *xMAv;
	PetscInt nloc;
    VecGetLocalSize(x,&nloc);
	VecGetArray(x,&xv);
    VecGetArray(xmin,&xmiv);
    ...
    VecGetArray(XMAX,&xMAv);
        for (PetscInt i=0;i<nloc;i++){
                xmav[i] = Min(xMAv[i], xv[i] + movlim);
                xmiv[i] = Max(xMIv[i], xv[i] - movlim);
        }
    VecRestoreArray(x,&xv);
    VecRestoreArray(xmin,&xmiv);
    VecRestoreArray(xmax,&xmav);
